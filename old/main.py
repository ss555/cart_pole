...
posChariot = 0
arrPosChariot = [posChariot]
posPendule = 300
arrPosPendule = [posPendule]
arrVitPendule = []
timePosPendule = [start_time]

def callbackPendule(way):
	global posPendule
	posPendule += way/300*math.pi
	arrVitPendule.append((posPendule-arrPosPendule[-1])/(time.time()-start_time))
	arrPosPendule.append(posPendule)
	timePosPendule.append(time.time()-start_time)
	info("pos pendule={}".format()#(posPendule/600)*2*math.pi))
...
def callbackChariot(way):
	...
	if posChariot > lengthLimit:
		...
		pi.set_PWM_dutycycle(24,  50) #max 255		
	elif posChariot==0 and homingRequest:
		homingRequest=False
		pi.set_PWM_dutycycle(24,  0)
...
ax.grid()
ax.plot(timePosPendule[1:], arrVitPendule)
