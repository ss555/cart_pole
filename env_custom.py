...
K2 = 0.05#0.15 #2 #0.1  #friction on theta
Mpoletest = 0.2
McartTest = 0.6
...
class CartPoleContinous(gym.Env):
    ...
    def _calculate_force(self,action):
        return -self.fa*self.state[1]+self.tensionMax*action+self.fc*np.sign(self.state[1])
    def step(self, action):
        [x,x_dot,costheta,sintheta,theta_dot]=self.state
        x_acc=(self._calculate_force(action)+self.masspole*self.g*sintheta*costheta-self.masspole*theta_dot**2*sintheta*self.length)/(self.masscart+self.masspole*self.sintheta**2)
        theta_acc=self.g/self.length*sintheta+x_acc/self.length*costheta
...
class CartPoleCosSinTensionD3(gym.Env):
    ...
    def step(self, action):#180 = 8.47V
        ...
        if self.kinematics_integrator=='euler':
            ...
            # x_dot=np.random.normal(x_dot,abs(x_dot/20),1)[0]
            # theta_dot=np.random.normal(theta_dot,abs(theta_dot/20),1)[0]
            # x_dot = np.random.normal(x_dot, abs(x_dot / 30), 1)[0]
...
class CartPoleCosSinTension(gym.Env):
    ...
    def __init__(self,
        ...
        self.masspole = Mpole
        self.total_mass = (self.masspole + self.masscart)
        self.length = 0.475  # actually half the pole's length
        self.polemass_length = (self.masspole * self.length)
