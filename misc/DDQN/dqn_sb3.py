...
from src.env_custom import CartPoleButter, CartPoleRK4
import argparse
...
LOAD_MODEL_PATH = './EJPH/dynamic-friction/dynamic_friction_sim_0.7035332644615993__best.zip' #None#SAVE_MODEL_PATH#'./weights/dqn50-sim/best_model_training.zip'#SAVE_MODEL_PATH#None#'./weights/dqn50-sim/best_model_training.zip'
LOAD_BUFFER_PATH = None #SAVE_BUFFER_PATH#"dqn_pi_swingup_bufferN"
logdir = './logs/'
VOLTAGE = 12 #8.4706
env = CartPoleRK4(Te=Te, N_STEPS=EP_STEPS,discreteActions=True,tensionMax=VOLTAGE, resetMode='experimental',sparseReward=False,Km=0.0,n=5,FILTER=False)
env = Monitor(env, filename=logdir+'basic_simulation_')

envEvaluation = CartPoleRK4(Te=Te,N_STEPS=EP_STEPS,discreteActions=True,tensionMax=VOLTAGE, resetMode='experimental',sparseReward=False,Km=0.0,n=5,FILTER=False)
