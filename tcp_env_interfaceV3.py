...
def getState():
    ...
    info('got state: {}'.format([posChariot, vChariot, posPendule, vPendule, done]))    
    return [posChariot, vChariot, posPendule, vPendule, done]
...
def reset():
	global posChariot,oldPosChariot,posPendule,oldPosPendule,old_time_chariot,old_time_pendule,done, reset_time_start ,RESET_TIME
	# GOTO center slowly
	info('reset')
	data_state = getState()
	x=data_state[0]
		...
		with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
			...
			while True:								
				...
				if str(data_action)=='RESET':
					...
					info('action applied{}'.format(data_action))
				old_time_chariot = time.time()
				old_time_pendule = time.time()
				time.sleep(0.048)#Te
