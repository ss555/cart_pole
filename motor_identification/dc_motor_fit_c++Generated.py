...
def integrate_acceleration(a,b,c,u,timeArr):
    ...
    for i in range(1,len(timeArr)):
        ...
        dv=a*v[i-1]+b*u[0]+c*np.sign(v[i-1])
        v[i]=v[i-1]+dv*dt
    return v
...
def plot_experimental_fitted(absPath,fA,fB,fC):
    ...
    for filename in names:
        ...
        v_fitted=integrate_acceleration(fA,fB,fC,u,timeArr=fileData[:,2])
        plt.plot(fileData[1:-1,2],v,'.',label=("%.1fV" % u[0]))
        plt.plot(fileData[:,2],v_fitted[:])
        plt.plot()
    plt.legend()
    plt.xlabel("Time, s")
...
print(len(expData))
print(error)
print(f'{-18.03005925191054,0.965036433340654,-0.8992003750802359}')
