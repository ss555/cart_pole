...
def create_bins(x_threshold,theta_dot_threshold):
    # obs[0] -> cart position --- -4.8 - 4.8
    # obs[1] -> cart velocity --- -inf - inf
    # obs[2] -> pole angle    --- -41.8 - 41.8
    # obs[3] -> pole velocity --- -inf - inf
    N_BINS=15
    bins = np.zeros((5, N_BINS))
    bins[0] = np.linspace(-x_threshold, x_threshold, N_BINS)
    bins[1] = np.linspace(-5, 5, N_BINS)
    bins[2] = np.linspace(-1, 1, N_BINS)
    bins[3] = np.linspace(-1, 1, N_BINS)
    bins[4] = np.linspace(-theta_dot_threshold, theta_dot_threshold, N_BINS)
